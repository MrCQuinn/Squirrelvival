...
public class AIPathDog : AIBase {
	...
	public float TrySearchPath () {
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && gameStarted && squirrelAlive && target != null) {
			SearchPath();
			return repathRate;
		} else {
	...
	public virtual void SearchPath () {
		...
		if(SquirrelController.inTree || Vector2.Distance (target.transform.position, transform.position) > 8f){
